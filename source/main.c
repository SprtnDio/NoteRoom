...
int main(void) {
    ...
    snprintf(admin_maclist_topic, sizeof(admin_maclist_topic), "%s/Admin/MacList", g_base_topic);
    ...
    while (aptMainLoop()) {
        ...
        if (game->isSyncing && (currentTime - game->bootTime > 7000)) {
        ...
        if ((game->appState == STATE_MAIN_MENU || game->appState == STATE_SUB_MENU) &&
            (currentTime - game->lastTopUiUpdate) > 500) {
            game->needsRedrawTop = true;
            game->needsRedrawBottom = true;
        ...
        if (game->appState == STATE_MAIN_MENU) {
            ...
            if (kDown & KEY_A) {
                if (game->isSyncing || isBanned(game->macAddress)) {
                    u64 now = osGetTime();
                    if (now - game->lastActionTime >= 500) {
                        ...
                        if (isBanned(game->macAddress)) {
                            ...
                            updateStatus("Syncing with server...", C2D_Color32(200, 150, 50, 255));
                        }
                    }
        ...
        if ((kDown & KEY_TOUCH)) {
            ...
            if (game->appState == STATE_SAVE_MENU || game->appState == STATE_LOAD_MENU) {
                ...
                if (touch.py >= TOOLBAR_Y_START) {
                    ...
                    else if (touch.px < 128) { if (actionAllowed()) { game->isEraser = false; game->needsRedrawBottom = true; } }
                    else if (touch.px < 192) { if (actionAllowed()) { game->isEraser = true; game->needsRedrawBottom = true; } }
                    else if (touch.px < 256) { if (actionAllowed()) { openKeyboard(); } }
                    else { if (actionAllowed()) { sendDrawing(); } }
                }
        ...
        if ((game->inChat || game->appState == STATE_SAVE_MENU || game->appState == STATE_LOAD_MENU) && (now - last_hb_time >= 2)) {
            char hb_topic[64];
            snprintf(hb_topic, sizeof(hb_topic), "%s/Heartbeat/C%d/S%d", g_base_topic, game->selectedCategoryIdx, game->selectedSubIdx);
            char hb_payload[128];
            snprintf(hb_payload, sizeof(hb_payload), "%s|%s|%s", game->clientID, game->userName, game->macAddress);
            mqtt_publish_qos(hb_topic, hb_payload, false, 1);
            last_hb_time = now;
